...
2. Verify resource purpose before assigning `kind`; datasets are not weights. GitHub, Hugging Face, ModelScope, and arXiv resource badges are generated automatically. Store plain-text Venue; rendering adds backticks. Use the shared modality palette instead of per-row badge URLs; new entries must describe actual inputs. Store source evidence, review depth, aliases, and unresolved details in `provenance`.
3. Run:
...
4. Review retained citations, placements, inputs, dates, and narrative/formulas. Keep prose concise; omit dated editing logs. Preserve collapsible sections, anchors outside `<details>`, and blank table boundaries. If conversion code changed, run `python3 -m unittest discover -s tests -v`. Formula or diagram edits additionally need MathJax/Mermaid and published-output checks where available.
